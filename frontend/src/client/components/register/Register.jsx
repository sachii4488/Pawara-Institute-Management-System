    ...
    const Formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            if(file){
                ...
                fd.append("institute_name",values.institute_name)
                fd.append("email",values.email)
                fd.append("owner_name",values.owner_name)
                fd.append("password", values.password)
               
                axios.post(`${baseUrl}/institute/register`,fd).then(resp=>{
...
<Box component={'div'} sx={{padding:'40px',maxWidth:"700px",margin:"auto"}} > 
    {message && <CustomizedSnackbars reset={resetMessage} type={type} message={message}/>}
        <Box sx={{ display: 'flex', justifyContent: "center", alignItems: "center", }} component={'div'}>
            <Typography variant="h3">Register Institute</Typography>
        </Box>
        <Paper  sx={{ padding: "20px", margin: "10px",minWidth:'320px' }}>
            ...
            >
                ...
                <Box component={'div'} sx={{display:"flex", flexDirection:"row",
                    ...
                    <Typography style={{marginRight:"50px"}}  variant="h4">Add Institute Picture</Typography>
             ...
             <TextField fullWidth sx={{ marginTop: "10px" }} id="filled-basic"
                    label="Institute Name " variant="outlined" name="institute_name"
                    value={Formik.values.institute_name}
                    onChange={Formik.handleChange}
                    onBlur={Formik.handleBlur} />
                {Formik.touched.institute_name && Formik.errors.institute_name&& <p style={{ color: "red", textTransform: "capitalize" }}>
                    {Formik.errors.institute_name}</p>}
