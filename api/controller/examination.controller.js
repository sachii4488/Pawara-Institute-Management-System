...
module.exports = {
    ...
    getExaminationByClass: async(req, res)=>{
        try {
            ...
            const examination = await Examination.find({class:req.params.classId,institute:instituteId}).populate("subject");
    ...
    updateExaminaitonWithId: async(req, res)=>{
        try {
            ...
            // const examinationAfterUpdate =await Institute.findOne({_id:id});
            res.status(200).json({success:true, message:"Examination Updated."})
        } catch (error) {
            
            console.log("Error in updateInstituteWithId", error);
            res.status(500).json({success:false, message:"Server Error in Update Institute. Try later"})
        }
